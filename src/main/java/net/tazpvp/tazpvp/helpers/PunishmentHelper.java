...
public class PunishmentHelper {
    ...
    public static void ban(OfflinePlayer target, String time, CommandSender commandSender, String reason) {
        ...
        if (commandSender instanceof Player p) {
            ChatHelper.announce(CC.RED + target.getName() + " was banned by " + p.getName(), Sound.BLOCK_STONE_BUTTON_CLICK_OFF);
        } else {
            ChatHelper.announce(CC.RED + target.getName() + " has been banned.", Sound.BLOCK_STONE_BUTTON_CLICK_OFF);
        }

        if (target.isOnline()) {
            Player banned = target.getPlayer();
            if (banned == null) return;
            ChatHelper.announce(banned, CC.GRAY + "You've been banned for: " + CC.RED + reason, Sound.BLOCK_ANVIL_LAND);
